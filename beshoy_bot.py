...
async def cb_proxy_choice(callback: CallbackQuery, state: FSMContext):
    ...
    if gate == "dark_post":
        ...
        await callback.message.edit_text("🌍 أدخل الدولة المستهدفة (مثل: EG, US, SA):", reply_markup=kb_back())
    elif gate == "page_like":
...
async def process_post_id(message: Message, state: FSMContext):
    text = message.text.strip()
    if not text.isdigit():
        await message.answer("❌ أدخل رقم البوست بس (أرقام فقط، بدون رقم الصفحة).", reply_markup=kb_back())
        return

    await state.update_data(post_id=text)
    await state.set_state(AdStates.waiting_budget)
    await message.answer("✅ تم حفظ رقم البوست.\n💰 أدخل الميزانية اليومية ($):", reply_markup=kb_back())
...
async def cb_confirm_yes(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        if gate == "dark_post":
            ...
            # المسار ده حاليًا مايمرّ مش على شاشات country/age/gender (تصميم الـ flow الأصلي)،
            # فلو القيم دي مش موجودة نستخدم استهداف افتراضي واسع بدل ما نطلع exception على None
            targeting = {
                "geo_locations": {"countries": [country] if country else ["EG"]},
                "age_min": int(age.split("-")[0]) if age else 18,
                "age_max": int(age.split("-")[1]) if age else 65,
                "genders": [1] if gender == "male" else [2] if gender == "female" else [1, 2],
                "targeting_automation": {"advantage_audience": 0}
            }
            if is_message_objective:
            ...
            # post_id لازم يكون بصيغة {page_id}_{post_id} عشان object_story_id يتقبل من فيسبوك
            full_post_id = post_id if "_" in post_id else f"{page_id}_{post_id}"
            if is_message_objective:
                creative = {
                ...
                creative = {"object_story_id": full_post_id}
